...
This command reads `instructions/prompt.md` from the package directory and writes it to stdout. Because `knowledge` is an **instruction skill**, this is its primary surface — there are no domain commands. The agent applies the methodology with its own `aux4 kb` calls (`add`, `search`, `view`, `list`, `update`, `remove`); the prompt does not run kb or call an LLM itself.
